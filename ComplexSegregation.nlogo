...
;; Properties of individual turtles
turtles-own [
...
;; This sets up the model by creating the appropriate number of turtles,
;;  determining a color for the turtle.
to setup
  ...
  ask patches [
    if random 100 < density [   ;; set the occupancy density
      sprout 1 [ ;; Make a turtle at this patch
        ifelse random 100 < Percent-red [  ;; Determine the turtles color
          set color red
        ]
        [
          set color green
...
to go
  ;; Don't run the model if everybody is happy
  if all? turtles [ happy? ] [ stop ]

  ;; Otherwise move and update
  move-unhappy-turtles
  update-variables

  ;; Advance the tick counter one tick
  tick
end
...
;; Updates the model after everyone has moved
to update-variables
  update-turtles
  update-globals
end
...
;; Updates the turtle status
to update-turtles
  ask turtles [

    ;; Switch types
    if random-float 1 < Probability-switch[
      set color ifelse-value (color = red) [green][red]
    ]

    ;; This variable defines what counts as a turtles "neighbors
    let relevant-turtles turtle-set nobody
    if Neighborhood = "Moore-Neighborhood" [ ;; are we using the Moore neighborhood
      set relevant-turtles turtles-on neighbors
    ]
    if Neighborhood = "Radius" [ ;; or the Radius
      set relevant-turtles turtles in-radius Radius
    ]

    ;; How many neighbors are similar to me?
    set similar-nearby count relevant-turtles  with [ color = [ color ] of myself ]
    set other-nearby count relevant-turtles with [ color != [ color ] of myself ]
    set total-nearby similar-nearby + other-nearby

    ;; Decide if I'm happy
    if color = red [
      set happy? (similar-nearby >= (red-minimum-wanted * total-nearby / 100)) and (similar-nearby <= (red-maximum-wanted * total-nearby / 100))
    ]
    if color = green [
      set happy?  (similar-nearby >= (green-minimum-wanted * total-nearby / 100)) and (similar-nearby <= (green-maximum-wanted * total-nearby / 100))
    ]

    ;; Mark me with an x if I'm unhappy
    ifelse happy? [ set shape "square" ] [ set shape "square-x" ]
  ]
end

;; Updates the global variables that record the status of the model
to update-globals
...
; Code by Uri Wilensky and Kevin Zollman
@#$#@#$#@
GRAPHICS-WINDOW
290
15
...
Portions copyright 1997 Uri Wilensky and 2018 Kevin Zollman
